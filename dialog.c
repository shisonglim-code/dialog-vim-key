/*
 * $Id: dialog.c,v 1.297 2026/01/01 13:22:13 tom Exp $
 *
 *  cdialog - Display simple dialog boxes from shell scripts
 *
 *  Copyright 2000-2025,2026	Thomas E. Dickey
 *
...
unescape_argv(int *argcp, char ***argvp)
{
	...
	my_argc = 0;
	if (special_argv != NULL) {
	    for (j = 0; special_argv[j] != NULL; ++j) {
	...
	if (!strcmp((*argvp)[j], "--")) {
		...
		if (fp) {
		    ...
		    *argcp += added - 2;
		    (*argvp)[*argcp] = NULL;	/* Write the NULL terminator */
...
process_trace_option(char **argv, int *offset)
{
    ...
    if (dialog_state.trace_output == NULL) {
...
#endif
	case o_iso_week:
	    if (dialog_vars.week_start == NULL) {	/* Monday is implied */
...
main(int argc, char *argv[])
{
	...
	case o_input_fd:
		...
		|| (dialog_state.input = fdopen(j, "r")) == NULL) {
		handle_leaks();
	...
	case o_output_fd:
	    if ((j = optionValue(argv, &offset)) < 0
		|| (dialog_state.output = fdopen(j, "w")) == NULL) {
		handle_leaks();
