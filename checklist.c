/*
 *  $Id: checklist.c,v 1.173 2026/01/04 15:56:04 tom Exp $
 *
	      ...
	      int *current_item)
{
  ...
  retry:
    ...
    adjust_size = 0;
    if (all.use_height <= 1 && automate_size)
	adjust_size = 1;
    if (all.use_height == 0) {
	...
	dlg_auto_size(title, prompt,
		      &height, &width,
		      MIN_HIGH + all.use_height - adjust_size, use_width);
    }
     ...
     */
    all.use_height = height - MIN_HIGH - cur_y - 2;
    if (specific_size)
	all.use_height++;
    if (all.use_height <= 0)
	all.use_height = 1;
