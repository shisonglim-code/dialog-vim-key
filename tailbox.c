/*
 *  $Id: tailbox.c,v 1.90 2026/01/01 19:21:18 tom Exp $
 *
...
#define ValidCol(n) ((n) <= limit && cols[(n)] >= 0)

	x = 1;
	for (i = 0; ValidCol(i) && cols[i] < obj->hscroll; ++i)
	    first = i;

	for (i = first; ValidCol(i) && ((cols[i] - cols[first]) < limit); ++i)
	    last = i;
